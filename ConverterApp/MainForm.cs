...
namespace ConverterApp
{
    public partial class MainForm : Form
    {
        ...
        private double ConvertUnit(double value, string type, string fromUnit, string toUnit)
        {
            ...
            Dictionary<(string, string), object> conversionFactors = new Dictionary<(string, string), object>
            {
                // Длина
                ...
                // Температура
                [("°C", "°F")] = new Func<double, double>(v => (v * 9 / 5) + 32), 
                [("°F", "°C")] = new Func<double, double>(v => (v - 32) * 5 / 9),
                [("°C", "K")] = new Func<double, double>(v => v + 273.15), 
                [("K", "°C")] = new Func<double, double>(v => v - 273.15),
                [("°F", "K")] = new Func<double, double>(v => (v + 459.67) * 5 / 9), 
                [("K", "°F")] = new Func<double, double>(v => (v * 9 / 5) - 459.67),
                // Скорость
            ...
            if (conversionFactors.ContainsKey(key))
            {
                ...
                if (factor is double d)
                    return value * d;
                else if (factor is Func<double, double> func)
                    return func(value);
                else
                    return value;
            }
